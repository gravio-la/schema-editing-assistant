...
import { StreamTestDemo } from './StreamTestDemo'

// ── Mock fetch that emulates the Vercel AI SDK v4 data stream protocol ────────
//
// Text token format:  0:"<json-stringified-char>"\n
// Finish format:      d:{"finishReason":"stop","usage":{...}}\n
  ...
  return async (_url, _opts) => {
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        // Small initial delay to show the loading indicator
...
const meta: Meta<typeof StreamTestDemo> = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Wires `useChat` to a mock data-stream endpoint. ' +
          'Type any message and hit Send to see simulated token-by-token streaming. ' +
          'To test against the real server, run `bun run dev:server` and open the story without the mock decorator.',
      },
    },
  },
}
